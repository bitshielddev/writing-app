...
The fresh schema persists projects, versioned block JSON plus Markdown, source metadata, a versioned suggestion projection, the committed versioned event outbox, and quarantined incompatible JSON. Durable document, source, and suggestion events use the `document:default-document` stream, retain a unique event ID and contiguous stream sequence, and remain available after dispatch for paginated replay. Per-consumer cursors record only contiguously applied acknowledgements; a dispatch timestamp is diagnostic, not delivery correctness state. It intentionally contains no provider settings, extracted-content index, agent memory, run transcript, or run-history tables. Temporary `suggestionPreview` blocks remain excluded from autosave. The complete version matrix and recovery policy are in [Compatibility](compatibility.md).
