...
function Keycap({ children }: { children: string }) {
  return (
    <kbd className="inline-flex min-h-6 min-w-6 items-center justify-center rounded border border-primary-foreground/20 bg-surface-raised/10 px-1.5 font-mono text-[0.7rem] font-bold text-primary-foreground shadow-sm">
      {children}
    </kbd>
  );
}
...
export function KeybindingCommandStrip({
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed bottom-5 left-1/2 z-[70] max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-xl bg-panel/95 px-3.5 py-2.5 text-primary-foreground shadow-2xl backdrop-blur"
    >
      {state.kind === "message" ? (
        ...
        <div className="flex max-w-[56rem] flex-wrap items-center justify-center gap-x-3 gap-y-2">
          ...
          <span className="hidden h-5 w-px bg-surface-raised/20 sm:block" aria-hidden="true" />
